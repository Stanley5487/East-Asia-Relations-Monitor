...
## 2026/08/28 - Agent v1 完成，發現URL生成異常問題

### 已完成
- 兩個工具(query_prediction, query_news)串接成功
- 透過system_prompt統一回答格式（現況摘要/詳細說明/參考來源三段式）
- 目前使用openai/gpt-oss-120b，後續若效能不佳可考慮使用付費模型

### 發現的問題
- LLM在生成回答時，偶爾會在複製URL這種長編碼字串時出現重複/卡住的異常生成，
  導致參考來源的網址無效
- 可能原因：讓LLM自己"記憶並複製"URL這種非自然語言字串，容易觸發生成式模型
  已知的重複生成問題

### 修正方向（進行中）
- 改用「代號引用」機制：query_news回傳時，不把完整URL放進context，
  改用[新聞1][新聞2]等代號標記；程式碼在Agent生成完回答後，
  用正則表達式掃描實際引用了哪些代號，再由程式碼組出正確的參考來源清單，
  不讓LLM自己生成URL文字

### 修正狀況
- 實作代號的方式後，發現LLM有時用[]有時用【】包住代號，導致正則表達式抓不到，
  改為只抓「新聞」後面的數字本身，不管符號種類。
- 發現_last_sources對照表在agent_core.py讀取時始終是空的，檢查後發現是
  query_news內部用`_last_sources = {}`重新賦值整個字典，導致agent_core.py
  當初import進來的參照與實際更新的物件不再同步；改用`.clear()`清空內容
  而非重新賦值後解決
- 目前已能穩定產出正確的APA格式參考來源（媒體. (年, 月日). 標題. URL）

### 下一步
- 持續優化Agent生成內容的品質與細節（風格、引用密度等）
- Streamlit整合，將Agent接入現有儀表板

## 2026/09/06 - agent前端部屬
### 已完成
整合預測系統與rag問答agent的前端
### 待修正
1. 多國語言檢索問題
2. LLM把工具名稱本身寫進回答（例如【query_prediction】）
3. 引用編號跳號問題（新聞1、新聞5,沒有2、3、4）

## 2026/0907 - 優化agent
### 已完成
1. 對話框排版問題
2. 透過prompt修正LLM把工具名稱寫進回答，改成參考本專案數據
3. 超過token上限改用訊息提示使用者，而不是報錯
4. 對話歷史紀錄記憶，但是有token容易爆量的問題
### 帶修正
1. 多國語言檢索問題
2. 未來可優化限制記憶的對話次數
